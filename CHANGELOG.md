...
上一轮把"收件箱"写进了文档，但脚本层面没有任何支撑：AI 只能靠手工翻文件夹，于是这个概念随时可能被忘掉或漏做。现在它是两条命令。

- **`Inbox`**：列出 `destinations.inbox` 文件夹里的全部条目，自动排除与文件夹同名的索引笔记（`00-收件箱/00-收件箱.md` 是对收件箱的说明，不是收件箱的内容）。每项给出收件箱内相对路径、大小、修改时间、扩展名、以及 `isPlainText`——能不能直接读，还是像 `.pdf`/`.docx` 那样需要转换器；判定先看扩展名（PDF 开头几百字节没有 NUL，光嗅探会误判成文本），再嗅探前 8KB 有无 NUL 字节，因此没有扩展名的随手文件也能判对。命中隐私片段的条目标 `sensitive`，与 `Next` 一样先给用户看再读。空文件夹单列在 `emptyDirectories`，不会因为"只列文件"而隐身。
- **`InboxClear -ItemIds <相对路径>`**：把已归位的条目移到 `controllerDataRoot\inbox-archive\<时间戳>\`，保持收件箱内的相对层级——**移动不删除**，和项目一贯的安全写入哲学一致，提炼错了原件还在。所有路径先全部解析校验再动手，最后一个 ID 打错不会留下清空到一半的收件箱；越出收件箱的路径、索引笔记、不存在的条目一律报错，不静默通过。只清理因这次移动而变空的子文件夹，本来就空的原样保留。
- 文档与实现对齐：`SKILL.md` 的 Core commands、`references/weekly-workflow.md`（收件箱从"第 0 步的约定"改成 Doctor 之后的一条命令）、`assets/scheduled-task-prompt.md`、两份 README 的命令表。
- 测试新增 9 项：排除索引笔记、纯文本判定、归档式清空、只清空被清空的文件夹、未知条目报错、坏 ID 导致整批中止、越界与索引笔记拒绝、整个文件夹作为一个条目清空且拒绝"文件夹与其内部文件同时清空"、未配置 `destinations.inbox` 时明确报错。

## Unreleased (2026-07-26)

### 笔记分级与分类（第五轮）

首位用户继续反馈两点：技术细节还要更少、要多聚焦自己的思考；以及"东西都被丢进工作项目了，其实里面有学习、有 AI 学习"。

- **重构章节结构**：删除 `## 怎么做的`（机制是用户最不想读的部分，工具最多在句中带一个短句），新增 `## 我学到了什么` 作为全篇重心且通常最长。整个知识库的意义被固定为用户自己的说法——「我学习了什么、我学会了什么、我做了什么」。
- **分类路由规则**：按"这条笔记的价值主要在哪"分流——完成/交付的事进 `04-工作项目`，学会的能力（尤其 AI 与工具的用法与边界）进 `03-AI学习`，备考材料进 `90-资料库`。产出了东西但留下来的价值是"我学会了怎么用 AI 做这件事"的，归 AI 学习。同类必须同处，否则用户找不到。
- **按项目活跃度决定篇幅**：以源文件夹的**活跃天数**（有多少个不同的日子被改动过）与最近更新时间分三档——持续推进的项目放宽到 4-6 KB 且进展记录按阶段写清"那个阶段学会了什么"；已完成的正式项目约 2 KB；一次性的事（帮朋友做的作业、跑完即弃的脚本）只写 0.5-1 KB，没有真教训时不强行编造"我学到了什么"。活跃天数比文件数更能区分"持续在做"与"集中做完"：一个 756 文件的项目若只有 3 个活跃日、两个月未动，它就是一次性交付。
